...
//    	System.out.println(CloudSim.clock()+":"+CloudSim.getEntityName(ev.getSource())+"-> FogScheduler = "+ev.getTag());
        switch (ev.getTag()) {
            ...
            case WorkflowSimTags.CLOUDLET_UPDATE:
				...
				case GWO:
					if(WorkflowEngine.updateFlag==0&&WorkflowEngine.startlastSchedule==0) {
						processCloudletUpdateForGWOInit(ev);
	            	}else if(WorkflowEngine.startlastSchedule==0){
	            		processCloudletUpdateForGWOUpdate(ev);
					}else {
						processCloudletUpdateForGWOAlphabest(ev);
					}
				  break;
				case GA:
...
//            System.out.println(cloudlet.getCloudletId()+".submit delay : "+delay);
    ...
    protected void processCloudletUpdateForGWOInit(SimEvent ev) {
    	List<Cloudlet> cloudletList=getCloudletList();
    	List<CondorVM> vmList=getVmsCreatedList();
//    	System.out.println("Hello"+engine.jobList.size());
    	if(GwoScheduling.initFlag==0) {
    		startTime = System.currentTimeMillis();
    		WorkflowEngine engine = (WorkflowEngine)CloudSim.getEntity(workflowEngineId);
    		GwoScheduling.init(engine.jobList.size(),getVmList().size());
    	}
    	List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
    	List<int[]> schedules=GwoScheduling.schedules;
    	for(int i=0;i<cloudletList.size();i++) {
    		int cloudletId=cloudletList.get(i).getCloudletId();
    		int vmId=schedules.get(count)[cloudletId];
    		cloudletList.get(i).setVmId(vmId);
    ...
    protected void processCloudletUpdateForGWOUpdate(SimEvent ev) {
    	List<Cloudlet> cloudletList=getCloudletList();
    	List<CondorVM> vmList=getVmsCreatedList();
    	if(WorkflowEngine.updateFlag2==1&&cloudletList.size()!=0) {
    		GwoScheduling.updateParticles();
    	}
    	List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
    	List<int[]> newSchedules=GwoScheduling.newSchedules;
    	for(int i=0;i<cloudletList.size();i++) {
    		int cloudletId=cloudletList.get(i).getCloudletId();
    		if(count2 >= newSchedules.size()) {
    			count2 = newSchedules.size() - 1;
    		}
    		if(count2 < 0) {
    			count2 = 0;
    		}
    		if(cloudletId >= newSchedules.get(count2).length) {
    			cloudletId = newSchedules.get(count2).length - 1;
    		}
    		if(cloudletId < 0) {
    			cloudletId = 0;
    		}
//    		System.out.println("yo yo");
    ...
    protected void processCloudletUpdateForGWOAlphabest(SimEvent ev) {
    	List<Cloudlet> cloudletList=getCloudletList();
    	List<CondorVM> vmList=getVmsCreatedList();
    	List<Cloudlet> scheduledList =new ArrayList<Cloudlet>();
//		System.out.println("yo yo 1");
    	for(int i=0;i<cloudletList.size();i++) {
    		int cloudletId=cloudletList.get(i).getCloudletId();
    		int vmId=GwoScheduling.alphabest_schedule[cloudletId];
    		cloudletList.get(i).setVmId(vmId);
    		//setVmState(vmId);
    		scheduledList.add(cloudletList.get(i));
    	}
