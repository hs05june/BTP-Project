    ...
    protected void processJobReturnForPSO(SimEvent ev) throws Exception {
        ...
        if(getJobsList().isEmpty() && jobsSubmitted == 0) {
        	...
        	if(index1 != PsoScheduling.particleNum) {
        		already = 1;
    			//A particle is processed （Initialized Particles）
             	FogBroker.count++;
                ...
                if(index1 == PsoScheduling.particleNum) {
 	        	...
 	        	//All initialized particles are processed, and a single particle is processed （Particles obtained after update）
        		int gbestIndex = 0;
            ...
            if(index1 == PsoScheduling.particleNum && indexForUpdate == PsoScheduling.particleNum) {
 	        	//All initialized particles are processed、Particles obtained after update
            	if(PsoScheduling.iterateNum > iterateNum) {
            		...
            		for(int i = 0; i < PsoScheduling.particleNum; i++) {  //Update global optimal
...
//	              	printindicators(PsoScheduling.gbest_fitness);
    ...
    protected void processJobReturnForGWO(SimEvent ev) throws Exception {
        ...
        if(getJobsList().isEmpty() && jobsSubmitted == 0) {
//    		System.out.println("-------------------------------------------");
        	if(index1 != GwoScheduling.particleNum) {
        		already = 1;
    			//A particle is processed （Initialized Particles）
//                System.out.println("uhuhruhfruhfruhurhuf"+ getJobsList().size());
             	FogBroker.count++;
             	fitness[index1++] = caculatefitness();
             	init();
                if(index1 == GwoScheduling.particleNum) {
        	...
        	else if(indexForUpdate != GwoScheduling.particleNum) {
 	        	//All initialized particles are processed, and a single particle is processed （Particles obtained after update）
        		int alphabestIndex = 0;
    			for(int i = 0; i < index1; i++) {
    				if(fitness[i] < GwoScheduling.alphabest_fitness) {
    					 GwoScheduling.alphabest_fitness = fitness[i];
    					 alphabestIndex = i;
    				}
    			}
    			GwoScheduling.alphabest_schedule = GwoScheduling.pbest_schedule.get(alphabestIndex);//Update the global optimal scheduling plan
    			
                int betabestIndex = 0;
    			for(int i = 0; i < index1; i++) {
    				if(fitness[i] < GwoScheduling.betabest_fitness && i != alphabestIndex) {
    					 GwoScheduling.betabest_fitness = fitness[i];
    					 betabestIndex = i;
    				}
    			}
    			GwoScheduling.betabest_schedule = GwoScheduling.pbest_schedule.get(betabestIndex);//Update the global optimal scheduling plan
    			

                int deltabestIndex = 0;
    			for(int i = 0; i < index1; i++) {
    				if(fitness[i] < GwoScheduling.betabest_fitness && i != alphabestIndex && i != betabestIndex) {
    					 GwoScheduling.deltabest_fitness = fitness[i];
    					 deltabestIndex = i;
    				}
    			}
    			GwoScheduling.deltabest_schedule = GwoScheduling.pbest_schedule.get(deltabestIndex);//Update the global optimal scheduling plan
    			
    			
                FogBroker.count2++;
	        	fitness2[indexForUpdate++] = caculatefitness();
	        	if(FogBroker.count2 != GwoScheduling.particleNum && GwoScheduling.iterateNum > iterateNum) {
	        		init();
	        		sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null);
	        	}
 		    }
            if(index1 == GwoScheduling.particleNum && indexForUpdate == GwoScheduling.particleNum) {
 	        	//All initialized particles are processed、Particles obtained after update
            	if(GwoScheduling.iterateNum > iterateNum) {
            		for(int i = 0; i < GwoScheduling.particleNum; i++) { 
            			//Update individual optimal
            			if(fitness[i] > fitness2[i]) {
            				int temp = i;
            				if(temp >= GwoScheduling.pbest_schedule.size()) {
            					temp = GwoScheduling.pbest_schedule.size() - 1;
            				}
            				else if(temp < 0) {
            					temp = 0;
            				}
            				if(temp >= GwoScheduling.newSchedules.size()) {
            					temp = GwoScheduling.newSchedules.size() - 1;
            				}
            				int schedule1[] = GwoScheduling.pbest_schedule.get(temp);
            				int schedule2[] = GwoScheduling.newSchedules.get(temp);
            				for(int j = 0; j < schedule1.length; j++)
            					schedule1[j] = schedule2[j];
            				GwoScheduling.pbest_fitness[i] = fitness2[i]; 
            			}
            			else
            				GwoScheduling.pbest_fitness[i]=fitness[i];
	        		}
            		fitness = GwoScheduling.pbest_fitness;
            		for(int i = 0; i < GwoScheduling.particleNum; i++) {  //Update global optimal
            			if(GwoScheduling.pbest_fitness[i] < GwoScheduling.alphabest_fitness) {
            				//index1=i;
            				GwoScheduling.alphabest_fitness = GwoScheduling.pbest_fitness[i];
            				GwoScheduling.alphabest_schedule = GwoScheduling.pbest_schedule.get(i);
            			}
            		}
                    for(int i = 0; i < GwoScheduling.particleNum; i++) {  //Update global optimal
            			if(GwoScheduling.pbest_fitness[i] < GwoScheduling.betabest_fitness && GwoScheduling.pbest_schedule.get(i) != GwoScheduling.alphabest_schedule) {
            				//index1=i;
            				GwoScheduling.betabest_fitness = GwoScheduling.pbest_fitness[i];
            				GwoScheduling.betabest_schedule = GwoScheduling.pbest_schedule.get(i);
            			}
            		}
                    for(int i = 0; i < GwoScheduling.particleNum; i++) {  //Update global optimal
            			if(GwoScheduling.pbest_fitness[i] < GwoScheduling.alphabest_fitness && GwoScheduling.pbest_schedule.get(i) != GwoScheduling.alphabest_schedule && GwoScheduling.pbest_schedule.get(i) != GwoScheduling.betabest_schedule) {
            				//index1=i;
            				GwoScheduling.deltabest_fitness = GwoScheduling.pbest_fitness[i];
            				GwoScheduling.deltabest_schedule = GwoScheduling.pbest_schedule.get(i);
            			}
            		}
            		iterateNum++;
            		System.out.println("After "+iterateNum+" iterations:");
...
//	              	printindicators(GwoScheduling.alphabest_fitness);
            		...
            		if(GwoScheduling.iterateNum != iterateNum) {
        	        	...
        	        	init(); 
         		    	updateFlag2 = 1;
         		    	GwoScheduling.newSchedules.removeAll(GwoScheduling.newSchedules);
         		    	sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null);
    	        	}
            	}
            }
            ...
            if(GwoScheduling.iterateNum == iterateNum && startlastSchedule == 0) {
            	for(int i = 0; i < GwoScheduling.particleNum; i++) {
            		if(GwoScheduling.pbest_fitness[i] == GwoScheduling.alphabest_fitness) {
            			GwoScheduling.alphabest_schedule = GwoScheduling.pbest_schedule.get(i);
            		}
